...
func mongoOf(c Config) mongoFields {
	return mongoFields{c.MongoUrl, c.MongoUser, c.MongoPassword, c.MongoAuthSource, c.MongoDatabase, c.MongoCollection}
}

// clearMongoEnv empties the variables for this test; the loader ignores empty values.
func clearMongoEnv(t *testing.T) {
	for _, k := range []string{"MONGO_URL", "MONGO_USER", "MONGO_PASSWORD", "MONGO_AUTH_SOURCE", "MONGO_DATABASE", "MONGO_COLLECTION", "MONGO_TABLE"} {
		t.Setenv(k, "")
	}
}
...
func TestLoad_RepoConfigMongoDefaults(t *testing.T) {
	...
	want := mongoFields{Url: "mongodb://localhost:27017", AuthSource: "admin", Database: "process_scheduler", Collection: "process_schedule"}
...
func TestLoad_MongoEnvNames(t *testing.T) {
	...
	t.Setenv("MONGO_USER", "process-scheduler")
	t.Setenv("MONGO_PASSWORD", "p@ss:w/rd")
	t.Setenv("MONGO_AUTH_SOURCE", "users")
	t.Setenv("MONGO_DATABASE", "process_scheduler_test")
	cfg, err := Load("../../config.json")
	if err != nil {
		t.Fatal(err)
	}
	want := mongoFields{
		...
		Password:   "p@ss:w/rd",
		AuthSource: "users",
		Database:   "process_scheduler_test",
		Collection: "process_schedule",
	}
	if got := mongoOf(cfg); got != want {
		t.Errorf("mongo = %+v, want %+v", got, want)
	}
...
func TestLoad_MongoTableNoLongerRead(t *testing.T) {
	...
	cfg, err := Load(writeConfig(t, `{"mongo_database": "process_scheduler", "mongo_table": "other"}`))
	if err != nil {
		t.Fatal(err)
	}
	if cfg.MongoDatabase != "process_scheduler" {
		t.Errorf("database = %q, want process_scheduler", cfg.MongoDatabase)
	}
}

func TestLoad_MongoConfigFile(t *testing.T) {
	clearMongoEnv(t)
	cfg, err := Load(writeConfig(t, `{"mongo_url": "mongodb://file:27017", "mongo_user": "u", "mongo_password": "s3cr3t", "mongo_auth_source": "a", "mongo_database": "d", "mongo_collection": "c"}`))
	if err != nil {
		t.Fatal(err)
	}
...
func TestLoad_EnvPrintMasksMongoPassword(t *testing.T) {
	...
	if !strings.Contains(out, "MONGO_USER  =  process-scheduler") {
...
func TestConfigFormattingMasksMongoPassword(t *testing.T) {
	...
	for name, s := range outputs {
		...
		if !strings.Contains(s, "process_schedule") {
